...
    if (!/^(0[3|5|7|8|9])([0-nine]{8})$/.test(phone)) return "Số điện thoại không hợp lệ (phải 10 số, bắt đầu bằng 0).";
    return "";
  };
...
      showNotify({ type: NotifyType.Error, title: "Lỗi", message: "Không thể tải danh sách vai trò." });
    }
  };
  const fetchUsers = async (page: number = 1) => {
    setIsLoading(true);
    try {
      const response = await UserService.getAllUsers(page, PAGE_LIMIT);
...
      showNotify({ type: NotifyType.Error, title: "Lỗi", message: "Không thể tải danh sách người dùng." });
    } finally {
      setIsLoading(false);
    }
  };
...
      showNotify({ type: NotifyType.Warning, title: "Thông tin không hợp lệ", message: "Vui lòng kiểm tra lại các trường báo lỗi." });
      return;
    }
    setAddFormErrors(initialErrors);
    
...
          response.data
        );
        showNotify({ type: NotifyType.Error, title: "Lỗi", message: "Tạo user thất bại, response lạ." });
        return;
      }
      
...
      }
      
      showNotify({ type: NotifyType.Success, title: "Thành công", message: "Đã thêm người dùng mới." });
...
          response.data
        );
        showNotify({ type: NotifyType.Error, title: "Lỗi", message: "Lấy chi tiết user thất bại." });
         return;
      }
...
      showNotify({ type: NotifyType.Error, title: "Lỗi", message: "Không thể tải chi tiết người dùng." });
    }
  };
...
      });
      
      showNotify({ type: NotifyType.Success, title: "Thành công", message: "Đã cập nhật người dùng." });
...
      showNotify({ type: NotifyType.Success, title: "Thành công", message: "Đã xóa người dùng." });
