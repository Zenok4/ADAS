...
export default function DashboardPage() {
  ...
  // 👉 Nút điều khiển: bật/tắt CẢ HAI (webcam + CameraLive)
  const handleCameraToggle = async () => {
    const anyOn = frontReady || signDetect;
    if (anyOn) {
      stopFront(); // tắt webcam
      setSignDetect(false); // tắt CameraLive
    } else {
      await openFront("webcam"); // bật webcam
      setSignDetect(true); // bật CameraLive (iVCam) — KHÔNG sửa CameraLive
    }
  };
